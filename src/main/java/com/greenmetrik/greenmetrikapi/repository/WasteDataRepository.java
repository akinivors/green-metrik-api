...
public interface WasteDataRepository extends JpaRepository<WasteData, Long>, JpaSpecificationExecutor<WasteData> {
    ...
    @Query("SELECT new com.greenmetrik.greenmetrikapi.dto.PublicStatsDTO$WasteSummary(SUM(w.organicProductionKg), SUM(w.inorganicProductionKg), SUM(w.toxicWasteKg)) FROM WasteData w WHERE w.dataDate >= :startDate AND w.dataDate <= :endDate")
